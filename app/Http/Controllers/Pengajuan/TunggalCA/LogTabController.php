...
// Form Request
use App\Http\Requests\Pengajuan\LogTabReq;
...
class LogTabController extends BaseController
{
    public function index(){
        $query = TabDebt::select('id', 'no_rekening', 'nama_bank', 'tujuan_pembukaan_rek', 'penghasilan_per_tahun', 'sumber_penghasilan', 'tujuan_pengeluaran_dana')->get();
    ...
    public function show($id){
        $query = TabDebt::where('id', $id)->first();
    ...
    public function update($id, LogTabReq $req){
        $check = TabDebt::where('id', $id)->first();

        if ($check == null) {
            return response()->json([
                'code'    => 404,
                'status'  => 'not found',
                'message' => "Data tabungan nasabah dengan id {$id} tidak ditemukan"
            ], 404);
        }
        ...
        try {
            TabDebt::where('id', $id)->update($dataTabUang);
